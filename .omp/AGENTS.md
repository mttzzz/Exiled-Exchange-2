...
- **Auto-updater шьётся на форк:** `main/electron-builder.yml` → `publish: {provider: github, owner: mttzzz, name: Exiled-Exchange-2}`. `AppUpdater.ts` логирует и показывает в UI реальный текст ошибки (`UpdateInfo.message` в `ipc/types.ts`); ссылки «О программе» идут на форк.
- **Docs (pages.yml)**: на форке GitHub Pages включается сам — `configure-pages` с `enablement: true` + `workflow_dispatch`. Сайт: `https://mttzzz.github.io/Exiled-Exchange-2/`.
